...
public class AgentPlacer : MonoBehaviour
{
    ...
    public void PlaceAgents()
    {
        ...
        for (int i = 0; i < dungeonData.rooms.Count; i++)
        {
            ...
            {
                room.PositionsAccessibleFromPath = new List<Vector2Int>(); // no entry = skip enemies
                continue;
            }

            Dictionary<Vector2Int, Vector2Int> roomMap = roomGraph.RunBFS(entryTile, room.PropPositions);
    ...
    private bool TryGetRoomEntry(Room room, out Vector2Int entry)
    {
        foreach (var tile in room.FloorTiles)
        {
            foreach (var d in dirs)
            {
                ...
                {
                    entry = tile;   // this room tile is adjacent to corridor
                    return true;
                }
            }
        }
